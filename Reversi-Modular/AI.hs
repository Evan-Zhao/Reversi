...
evaluate color board = occupyScore - (if noMove (AI color) board then noMovePenalty else 0) where
...
-- This function uses color instead of player since ai does not care whether
-- the opponent is human or not.
...
retrace aic presentc depth board
    ...
    | otherwise      = choiceFun $ zip subScores subIdx where
        ...
        subBoards               = map extract subBoardsJust where extract (Just x) = x
        (subBoardsJust, subIdx) = unzip $ filter (\(b, i) -> b /= Nothing) $
                                zip (map (moveAt (AI presentc) board) indices) indices

ai :: Color -> Board -> (Board, Indice)
ai aic board = (extract $ moveAt (AI aic) board bestMove, bestMove) where
